...
module LightingTechnique (
  ...
  , setLightingColorTextureUnit
  , setLightingDisplacementMapTextureUnit
  , setDirectionalLight
  , setEyeWorldPos
  , setMatSpecularPower
...
data LightingTechnique =
    ...
    , lDispFactorLoc                    :: !UniformLocation
    }
...
initLightingTechnique = do
    ...
    addShader program "tutorial30/lighting.vs" VertexShader
    addShader program "tutorial30/lighting.fs" FragmentShader
    addShader program "tutorial30/lighting.cs" TessControlShader
    addShader program "tutorial30/lighting.es" TessEvaluationShader
    finalize program

    vpLoc <- getUniformLocation program "gVP"
    worldMatrixLoc <- getUniformLocation program "gWorld"
    samplerLoc <- getUniformLocation program "gColorMap"
    displacementMapLoc <- getUniformLocation program "gDisplacementMap"
    eyeWorldPosition <- getUniformLocation program "gEyeWorldPos"

    dispFactorLoc <- getUniformLocation program "gDispFactor"
    ...
    return LightingTechnique
        ...
        , lDisplacementMapLoc               = displacementMapLoc
...
setLightingVP :: LightingTechnique -> [[GLfloat]] -> IO ()
setLightingVP LightingTechnique{..} mat = uniformMat lVPLoc $= mat
...
setLightingColorTextureUnit :: LightingTechnique -> GLuint -> IO ()
setLightingColorTextureUnit LightingTechnique{..} textureUnit =
    uniformScalar lSamplerLoc $= textureUnit

setLightingDisplacementMapTextureUnit :: LightingTechnique -> GLint -> IO ()
setLightingDisplacementMapTextureUnit LightingTechnique{..} textureUnit =
    uniformScalar lDisplacementMapLoc $= textureUnit
...
setDispFactor :: LightingTechnique -> GLfloat -> IO ()
setDispFactor LightingTechnique{..} factor =
    uniformScalar lDispFactorLoc $= factor
